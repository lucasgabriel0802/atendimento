...
if (isset($_POST["a"])) {
    ...
    if(($acao == "reprocessar-e-social")){
        ...
        $oc->deletarOcorrencias($_SESSION[config::getSessao()]["unidade"],
    ...
    if (($acao == "pesquisar-e-social-s2210") && $parametro->getHabilitaGeracaoeSocial()) {
        ...
        $filtro = ["A.UNIDADE" => $_SESSION[config::getSessao()]["unidade"],
                     "A.EMPRESA" => $_SESSION[config::getSessao()]["empresa_ativa"]];
                ...
                if ($es->getItemLista($i)->getTransmiteESocial() == "S") {
                    ...
                    if($es->getItemLista($i)->getSituacaoTransmissao() == "R"){
                        $botoes = "<button id=\"btn-reprocessar-".$es->getItemLista($i)->getChave()."\" ".
                                  ...
                                  "<i class=\"fa fa-repeat\"></i> Reprocessar</button> ";

                    }else{
                        $botoes = "<a class=\"btn btn-default btn-xs disabled \"  href=\"javascript: void(0)\">Reprocessar</a>";
                    }
                    $reg->addItem($botoes, array("style" => "text-align: center;"));
    ...
    if (($acao == "pesquisar-e-social-s2220") && $parametro->getHabilitaGeracaoeSocial()) {
        ...
        if ($es->buscar($filtro,
            ...
            $cab->addItem("Seq", array("style" => "font-weight: bold; text-align: center; width: 5%"));
            $cab->addItem("Funcionário", array("style" => "font-weight: bold; text-align: center;"));
            $cab->addItem("Data/hora", array("style" => "font-weight: bold; text-align: center; width: 10%"));
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#filtro-input-data-inferior').datepicker({autoclose: true});
        $('#filtro-input-data-inferior').mask('99/99/9999');

        $('#filtro-input-data-superior').val(dataFinal);
        $('#filtro-input-data-superior').datepicker({autoclose: true});
        $('#filtro-input-data-superior').mask('99/99/9999');

        // pesquisarESocial('1', 's1060');
        pesquisarESocial('1', 's2210', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
        pesquisarESocial('1', 's2220', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
        // pesquisarESocial('1', 's2221');
        pesquisarESocial('1', 's2240', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
    ...
    $('#form-button-pesquisar-s1060').on('click', function() {
        pesquisarESocial('1', 's1060', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
    });
    ...
    $('#form-button-pesquisar-s2221').on('click', function() {
        pesquisarESocial('1', 's2221', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
    });
    ...
    $('#form-button-pesquisar-s2245').on('click', function() {
        pesquisarESocial('1', 's2245', $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
    });
    ...
    function baixarTodosAbertos(origem) {
        ...
        if ($('#div-e-social-' + origem).find('a[attr="baixar-esocial"][situacao="A"]').length) {
            ...
            setTimeout(function() {
                pesquisarESocial('1', origem, $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
            }, 100);
        }
    }

    function pesquisarESocial(pagina, origem, dataInicial, dataFinal) {
        paginaAtual = pagina;
            ...
            $('#div-e-social-' + origem + ' a').each(function() {
                if (origem == 's1060') {
                    $(this).on('click', function() {
                        ...
                        setTimeout(function() {
                            pesquisarESocial(paginaAtual, origem, $('#filtro-input-data-inferior').val(), $('#filtro-input-data-superior').val());
                        }, 300);
                    });
                } else {
                    $(this).on('click', function() {
    ...
    function reprocessar(chave) {
        ...
        $.ajax({
            ...
            $.gritter.add({ title: 'Oba!', text: 'O registro foi enviado para reprocessamento. Atualize a página em alguns instantes!', class_name: 'success' });
