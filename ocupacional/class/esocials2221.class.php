...
class esocials2221 implements iface {
    private $funcionario = "";
    private $sequencial  = "";
    private $datahora    = "";
    private $xml         = "";
    ...
    function gerarXML($unidade, $empresa, $matricula, $sequencial, $usuario, $xml) {
        ...
        $evtMonit = $dom->getElementsByTagName("evtToxic");
        $id = $evtMonit->item(0)->attributes->item(0)->nodeValue;
        
        if (!empty($id)) {
            $fb = new FB("T_ESOCIAL_S2221");
        ...
        header('Content-disposition: attachment; filename="' . $id . '-S2221.xml"');
