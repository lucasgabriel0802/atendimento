            ...
            if ($ti->getSituacao() == "A"){
                ...
                if ($diasEmAtraso <= 30){
                    ...
                    if ($ti->getPercMoraDia() > 0){
                        ...
                        $boleto->PercentualJurosDiaAtraso = $ti->getPercMoraDia();
                    }else{
                        $percMoradia = $cb->getMoraDia();
                        //$boleto->PercentualJurosDiaAtraso = number_format($cb->getMoraDia(), 2, ",", ".");
                        $boleto->PercentualJurosDiaAtraso = $cb->getMoraDia();
                    }
                    ...
                    if ($ti->getPercMultaFixa() > 0){
                        ...
                        $boleto->PercentualMultaAtraso = $ti->getPercMultaFixa();
                    }else{
                        $percMultaFixa = $cb->getMultaFixa();
                        //$boleto->PercentualMultaAtraso = number_format($cb->getMultaFixa(), 2, ",", "."); 
                        $boleto->PercentualMultaAtraso = $cb->getMultaFixa();
                    }
                    ...
                    //$milhar = localeconv()["thousands_sep"];
                    //$decimal = localeconv()["decimal_point"];
                    
                    //$boleto->ValorDocumento = number_format($ti->getValorTitulo() + $valorAcrescimo, 2, $decimal, $milhar);        
                    $boleto->ValorDocumento = $ti->getValorTitulo() + $valorAcrescimo;
