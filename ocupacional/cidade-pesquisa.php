<?php
    ...
    if (isset($_POST["a"])){
        ...
        if ($acao == "pesquisa"){
            ...
            if ($descricao != "")
                $filtro["A.NOME"] = array($descricao, "CONTAINING");
            
            $cidade = new cidade();
            if ($cidade->buscar($filtro, $limite, array("A.NOME" => "ASC"))){
                $totalRegistros = $cidade->getTotalRegistros();
                ...
                $cab->addItem("UF", array("style" => "width: 15px; font-weight: bold"));
                $cab->addItem("Descrição", array("style" => "font-weight: bold"));
                $cab->addItem("", array("style" => "width: 20px"));
                $tab->addCabecalho($cab);
                
                for ($i = 0; $i < $cidade->getTotalLista(); $i++){
                    $reg = new registro(array("class" => "no-border-y"));
                    $reg->addItem($cidade->getItemLista($i)->getId());
                    $reg->addItem($cidade->getItemLista($i)->getIdUF());
                    $reg->addItem(utf8_encode($cidade->getItemLista($i)->getNome()));
                    $reg->addItem("<a class=\"btn btn-primary btn-xs\" href=\"javascript: void(0)\" "
                                  . "codigo=\"{$cidade->getItemLista($i)->getId()}\""
                                  . "UF=\"{$cidade->getItemLista($i)->getIdUF()}\""
                                  . "nome=\"".utf8_encode($cidade->getItemLista($i)->getNome())."\"><i class=\"fa fa-check\"></i></a>");
                    $tab->addRegistro($reg);
                }
                echo $tab->gerar();
                funcoes::gerarPaginacao(($pagina + 1), $totalRegistros, "pesquisarCidade('<pagina>');");
            }else
                                                     ...
                                                     'pagina': pagina 
                                                        ...
                                                        $('#modal-div-pesquisa a').each(function(){
                                                            $(this).on('click', function(){
                                                                ...
                                                                ?>
                                                                $('#<?php echo $prefixo; ?>-codigo-ibge').val($(this).attr('codigo'));
                                                                $('#<?php echo $prefixo; ?>-select-uf').val($(this).attr('uf'));
                                                                $('#<?php echo $prefixo; ?>-cidade').val($(this).attr('nome'));
