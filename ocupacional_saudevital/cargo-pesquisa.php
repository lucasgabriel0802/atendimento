<?php
    ...
    if (isset($_POST["a"])){
        ...
        if ($acao == "pesquisa"){
            ...
            $ca = new cargo();
            if ($ca->buscar($filtro, $limite, array("A.DESCRICAO" => "ASC"))){
                ...
                echo $tab->gerar();
                funcoes::gerarPaginacao(($pagina + 1), $totalRegistros, "pesquisarCargo('<pagina>');");
            }else
...
<div class="form-group-sm">
    <div class="col-md-12">
        ...
        <div class="input-group">
            <input type="input" id="modal-input-descricao-cargo" class="form-control input-sm">
...
<div id="modal-div-pesquisa-cargo"></div>
<script>
    $('#modal-button-pesquisar').on('click', function(){
        pesquisarCargo('1');
    });
    $('#modal-input-descricao-cargo').keypress(function(event){
        if (event.which == 13)
    ...
    function pesquisarCargo(pagina){
        $('#modal-div-pesquisa-cargo').html('<center><div style="top: 30px; position: inherit;"><img src="img/loader2.gif"></div></center>');
        $('#modal-div-pesquisa-cargo').load('cargo-pesquisa.php', 
                                                    {'a': 'pesquisa', 
                                                     'descricao': $('#modal-input-descricao-cargo').val(), 
                                                     'pagina': pagina 
                                                    }, function(){
                                                        $('#modal-div-pesquisa-cargo a').each(function(){
                                                            $(this).on('click', function(){
                                                                ...
                                                                ?>
                                                                $('#<?php echo $prefixo; ?>-input-cargo').val($(this).attr('codigo'));
                                                                $('#<?php echo $prefixo; ?>-input-descricao-cargo').val($(this).attr('descricao'));
