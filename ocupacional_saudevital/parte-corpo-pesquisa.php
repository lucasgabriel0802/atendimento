<?php
    ...
    require_once("class/partecorpo.class.php");
    ...
    if (isset($_POST["a"])){
        ...
        if ($acao == "pesquisa"){
            ...
            if ($partecorpo->buscar($filtro, $limite, array("A.DESCRICAO" => "ASC"))){
                $totalRegistros = $partecorpo->getTotalRegistros();
                ...
                for ($i = 0; $i < $partecorpo->getTotalLista(); $i++){
                    $reg = new registro(array("class" => "no-border-y"));
                    $reg->addItem($partecorpo->getItemLista($i)->getCodigo());
                    $reg->addItem(utf8_encode($partecorpo->getItemLista($i)->getDescricao()));
                    $reg->addItem("<a class=\"btn btn-primary btn-xs\" href=\"javascript: void(0)\" "
                                  . "codigo=\"{$partecorpo->getItemLista($i)->getCodigo()}\""
                                  . "descricao=\"".utf8_encode($partecorpo->getItemLista($i)->getDescricao())."\"><i class=\"fa fa-check\"></i></a>");
                    $tab->addRegistro($reg);
                }
                echo $tab->gerar();
...
<script>
    ...
    function pesquisar(pagina){
        ...
        $('#modal-div-pesquisa').load('parte-corpo-pesquisa.php', 
                                                    {'a': 'pesquisa', 
                                                     ...
                                                     'pagina': pagina 
                                                        ...
                                                        $('#modal-div-pesquisa a').each(function(){
                                                            $(this).on('click', function(){
                                                                ...
                                                                ?>
                                                                $('#<?php echo $prefixo; ?>-codigo-parte-corpo').val($(this).attr('codigo'));
                                                                $('#<?php echo $prefixo; ?>-descricao-parte-corpo').val($(this).attr('descricao'));
