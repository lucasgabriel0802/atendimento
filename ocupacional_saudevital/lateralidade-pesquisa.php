<?php
    ...
    require_once("class/lateralidade.class.php");
    ...
    if (isset($_POST["a"])){
        ...
        if ($acao == "pesquisa"){
            ...
            $lateralidade = new lateralidade();

            if ($lateralidade->buscar($filtro, $limite, array("A.DESCRICAO" => "ASC"))){
                $totalRegistros = $lateralidade->getTotalRegistros();
                ...
                for ($i = 0; $i < $lateralidade->getTotalLista(); $i++){
                    $reg = new registro(array("class" => "no-border-y"));
                    $reg->addItem($lateralidade->getItemLista($i)->getCodigo());
                    $reg->addItem(utf8_encode($lateralidade->getItemLista($i)->getDescricao()));
                    $reg->addItem("<a class=\"btn btn-primary btn-xs\" href=\"javascript: void(0)\" "
                                  . "codigo=\"{$lateralidade->getItemLista($i)->getCodigo()}\""
                                  . "descricao=\"".utf8_encode($lateralidade->getItemLista($i)->getDescricao())."\"><i class=\"fa fa-check\"></i></a>");
                    $tab->addRegistro($reg);
                }
                echo $tab->gerar();
...
<script>
    ...
    function pesquisar(pagina){
        ...
        $('#modal-div-pesquisa').load('lateralidade-pesquisa.php', 
                                                    {'a': 'pesquisa', 
                                                     ...
                                                     'pagina': pagina 
                                                        ...
                                                        $('#modal-div-pesquisa a').each(function(){
                                                            $(this).on('click', function(){
                                                                ...
                                                                ?>
                                                                $('#<?php echo $prefixo; ?>-codigo-lateralidade').val($(this).attr('codigo'));
                                                                $('#<?php echo $prefixo; ?>-descricao-lateralidade').val($(this).attr('descricao'));
