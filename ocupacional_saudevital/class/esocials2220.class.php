...
class esocials2220 implements iface
{
    private $funcionario = "";
    private $sequencial  = "";
    private $datahora    = "";
    ...
    function getSituacaoTransmissao() {return $this->situacao_transmissao; }
    function getTransmiteESocial() {return $this->transmite_esocial; }
    ...
    function setSituacaoTransmissao($valor){$this->situacao_transmissao = $valor;}
    function setTransmiteESocial($valor){$this->transmite_esocial = $valor;}
    function setChave($valor){$this->chave = $valor;}
    ...
    function buscar($filtro = null, $limite = null, $ordem = null)
    {
        $select = array(
            ...
            "E.TRANSMISSAOESOCIAL", "A.CHAVE", "C.RETORNORECIBO, A.DATAASO"
        );
        $join   = "JOIN T_FUNCIONARIO B ON B.MATRICULA = A.MATRICULA "
            . "                    AND B.EMPRESA   = A.EMPRESA   "
            . "                    AND B.UNIDADE   = A.UNIDADE   "
            . "LEFT OUTER JOIN T_ESOCIAL C ON C.EVENTO     = 'S2220' "
            . "                 AND C.CHAVE     = A.CHAVE   "
        ...
        $fb   = new FB("T_ESOCIAL_S2220 A");
        $ret = $fb->select($filtro, null, null, $join, $select, null, false);
        $this->totalRegistros = count($ret);

        $ret = $fb->select($filtro, $limite, $ordem, $join, $select, null, false);
    ...
    function gerarXML($unidade, $empresa, $matricula, $sequencial, $usuario, $xml)
    {
        ...
        $evtMonit = $dom->getElementsByTagName("evtMonit");
        $id = $evtMonit->item(0)->attributes->item(0)->nodeValue;

        if (!empty($id)) {
            $fb = new FB("T_ESOCIAL_S2220");
            $fb->update(
        ...
        if ($empresa->getModeloArquivoDownload() == 1) {
            ...
            $nome = $filial . '_S-2220_' . $filial . $sequencia . 'S2220' . date("Ymd", time()) . '.xml';
        }
