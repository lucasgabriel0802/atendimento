...
class esocials2210 implements iface
{
    private $funcionario = "";
    private $sequencial  = "";
    private $datahora    = "";
    ...
    function buscar($filtro = null, $limite = null, $ordem = null)
    {
        ...
        $join   = "JOIN T_FUNCIONARIO B ON B.MATRICULA = A.MATRICULA "
            ...
            . "LEFT JOIN T_ESOCIAL C ON C.EVENTO     = 'S2210' "
            . "                 AND C.CHAVE     = A.CHAVE   "
    ...
    {
        $val["XML"] = '<?xml version="1.0" encoding="UTF-8"?>' . $val["XML"];
        if ($val != null) {
            if (is_array($val)) {
                $fb = new FB("T_ESOCIAL_S2210");
    ...
    function gerarXML($unidade, $empresa, $matricula, $sequencial, $usuario, $xml)
    {
        ...
        $evtCAT = $dom->getElementsByTagName("evtCAT");
        $id = $evtCAT->item(0)->attributes->item(0)->nodeValue;
        ...
        if ($empresa->getModeloArquivoDownload() == 1) {
            ...
            $nome = $filial . '_S-2210_' . $filial . $sequencia . 'S2210' . date("Ymd", time()) . '.xml';
        }
